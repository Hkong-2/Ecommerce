...
## QUY TRÌNH HOẠT ĐỘNG & THỰC THI
1. Quét cây thư mục (Directory Tree).
2. Tự động áp dụng **IGNORE RULES** để lọc bỏ các file rác/file tĩnh/file lock.
3. **KHÔNG CẦN TEST:** Trong quá trình viết code, sửa lỗi hay tối ưu hóa, bạn **chỉ cần đảm bảo đúng logic nghiệp vụ và không gây lỗi build (build error)**. Tuyệt đối không cần giả lập môi trường chạy thử (runtime simulation) hoặc viết test case để tiết kiệm tối đa thời gian xử lý và tài nguyên.